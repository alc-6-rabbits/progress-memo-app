# TASK-20260322-1712: BRIEFING 画面実装 + ナビゲーション更新

## 1. 概要

`DESIGN-20260322-1411.md` Phase 2-2 に基づき、統合ダッシュボード画面（MISSION BRIEFING）を新規作成する。ローカルタスクと GitHub Projects V2 のデータを統合表示する。あわせて、`layouts/default.vue` のナビゲーションに BRIEFING リンクを追加する。

> **前提条件**: TASK-20260322-1711（useGitHubGraphQL composable）が完了していること
> **参照**: [REQ-20260322-1327](../requirements/REQ-20260322-1327.md) FR-001, [DESIGN-20260322-1411](../designs/DESIGN-20260322-1411.md) §4.1, §6

## 2. 実装詳細

### 2.1. `pages/briefing.vue` 新規作成
...
const githubActiveItems = computed(() => {
  return githubAllItems.value.filter(i => 
    i.status && i.status.toLowerCase() !== 'done'
  )
})
...
### 2.2. `layouts/default.vue` ナビゲーション更新

現在のナビゲーション（13-19行）に BRIEFING リンクを追加する。
...
**注意**: ナビ項目が6つに増えるため、`space-x-8` を `space-x-6` に縮小して収まりを確保する。

### 2.3. 起動画面分岐の追加

`pages/settings.vue` の startupScreen セレクト（96-99行）に `briefing` オプションを追加する。

**変更前**:

```html
<select v-model="startupScreen" class="...">
...
`pages/index.vue` の `onMounted` 内に起動画面リダイレクト処理を追加する。

```javascript
// index.vue の onMounted 先頭に追加
const startupScreen = localStorage.getItem('startupScreen') || 'tasks'
if (startupScreen === 'briefing') {
  navigateTo('/briefing')
  return
} else if (startupScreen === 'report') {
  navigateTo('/report')
  return
}
```

> **注意**: `navigateTo` は Nuxt 3 のグローバルヘルパー。import 不要。

## 3. 成果物 (Deliverables)

- [ ] `pages/briefing.vue` の新規作成
- [ ] `layouts/default.vue` のナビゲーション更新
...
- ブランチ名: `feature/phase2-briefing-dashboard`
- コミットメッセージ: `feat: add MISSION BRIEFING dashboard with GitHub Projects integration`

## 6. 次のアクション

- TASK-20260322-1713（GitHub Activity 日報取込）は本タスクと並行して着手可能
